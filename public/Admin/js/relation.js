                ...
                $("body").on("click", ".searchLecturer", function(){
                    ...
                    if ( !(name == '') ) {
                        loadUrl('relationteacher?q=' + encodeURI(name))
                    }
                })
                ...
                function sureSelectLecturer() {
                    ...
                    $("#relationapp-page input:checked[name=aids]").each(function(){
                        ...
                        aids += $(this).val();
                        lecturerName += $.trim(tr.find("td").eq(3).text());
                    })
                    if(aids != ''){
                        $("#relationapp").removeAttr("readonly").val(lecturerName).attr("readonly", "readonly");
                         var cls =  $("#relationapp").parent().next("input").attr('class');
                         $("#relationapp").parent().next("input").remove();
                         $("#relationapp").next('input').remove();
                        $("<input type='hidden' class='"+cls+"' name='"+$("#relationapp").attr("name")+"' value='"+aids+"' />").insertAfter($("#relationapp"));
                    }else{
                        alert("请选择讲师");
                    }
                }
