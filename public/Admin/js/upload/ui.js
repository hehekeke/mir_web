...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div id='url'><strong>Link:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
        ...
        var name = 'attachment';
        str = "<div  id='url'><strong>Link:</strong><a href=" + link + " target='_blank' > " + res.key + "</a></div>";
        // i++;
    ...
    //清空上传状态，成功之后显示链接
    $('#ppt_upload').hide();
    $('#fusuccess').removeAttr('style').children('a').attr('href',link);
    $('#fusuccess').children('a').text(link);
    $('#attachment').val('');//给input hidden赋value值
    $('#attachment').val(link);//给input hidden赋value值
    $('#pickfiles').children('span').text('重新上传');
    ...
    /////////////////////////////////////////////////////
    // progressNameTd.append(Wrapper);上传完成后显示的图片 //
    /////////////////////////////////////////////////////
...
FileProgress2.prototype.setTimer = function(timer) {
    this.fileProgressWrapper.FP_TIMER = timer;
};

FileProgress2.prototype.getTimer = function(timer) {
    return this.fileProgressWrapper.FP_TIMER || null;
};

FileProgress2.prototype.reset = function() {
    this.fileProgressWrapper.attr('class', "progressContainer");
    this.fileProgressWrapper.find('td .progress .progress-bar-info').attr('aria-valuenow', 0).width('0%').find('span').text('');
    this.appear();
};

FileProgress2.prototype.setChunkProgess = function(chunk_size) {
    var chunk_amount = Math.ceil(this.file.size / chunk_size);
    if (chunk_amount === 1) {
        return false;
    }
...
FileProgress2.prototype.setProgress = function(percentage, speed, chunk_size) {
    this.fileProgressWrapper.attr('class', "progressContainer green");
...
FileProgress2.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        ...
        url = domain + encodeURI(res.key);
        var link = 'http://'+domain + '/'+res.key;
        var name = 'multipleFile[]';
        str = "<div  id='url'><strong>Link:</strong><a href=" + link + " target='_blank' > " + link + "</a></div>";
        // i++;
    ...
    //清空上传状态，成功之后显示链接
    $('#video_upload').hide();
    $('#vusuccess').removeAttr('style').text(link);
    $('#video_attachement').val('');
    $('#video_attachement').val(link);//给input hidden赋value值
    $('#pickfiles2').children('span').text('重新上传');
    var progressNameTd = this.fileProgressWrapper.find('.progressName');
    var imageView = '?imageView2/1/w/100/h/100';
...
FileProgress2.prototype.setCancelled = function(manual) {
    var progressContainer = 'progressContainer';
    if (!manual) {
        progressContainer += ' red';
    }
...
FileProgress2.prototype.setStatus = function(status, isUploading) {
    if (!isUploading) {
        this.fileProgressWrapper.find('.status').text(status).attr('class', 'status text-left');
    }
};
...
// 绑定取消上传事件
FileProgress2.prototype.bindUploadCancel = function(up) {
    var self = this;
    if (up) {
        self.fileProgressWrapper.find('td:eq(2) .progressCancel').on('click', function(){
            self.setCancelled(false);
...
FileProgress2.prototype.appear = function() {
    if (this.getTimer() !== null) {
        clearTimeout(this.getTimer());
        this.setTimer(null);
    }
